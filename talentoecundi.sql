...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 20-10-2024 a las 17:56:10
...
--
DELIMITER $$
CREATE TRIGGER `after_inscripcion_insert` AFTER INSERT ON `inscripcion` FOR EACH ROW BEGIN
    DECLARE v_id_usuario INT;
    DECLARE v_titulo VARCHAR(20);
    DECLARE v_mensaje VARCHAR(150);

    -- Obtener el id_usuario del dueño de la oferta (empresa)
    SELECT u.id
    INTO v_id_usuario
    FROM oferta o
    ...
    -- Definir el título y mensaje de la notificación
    SET v_titulo = 'Nueva Inscripción';
    SET v_mensaje = CONCAT('Un egresado se ha inscrito a la oferta con el cargo: ', (SELECT cargo FROM oferta WHERE id = NEW.id_oferta), '.');

    -- Insertar la notificación
    INSERT INTO notificacion (id_usuario, titulo, mensaje, fecha, estado)
    VALUES (v_id_usuario, v_titulo, v_mensaje, NOW(), 'No Leída');
END
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `tr_pasar_segunda_fase_notificacion` AFTER UPDATE ON `inscripcion` FOR EACH ROW BEGIN
  -- Verificar si el estado cambió a 'En proceso'
  IF NEW.estado = 'En segunda fase' AND OLD.estado <> 'En segunda fase' THEN
    -- Obtener el ID de usuario del egresado y almacenarlo en una variable de usuario
    SET @egresado_usuario_id = (SELECT id_usuario FROM egresado WHERE id = NEW.id_egresado);

    -- Obtener el cargo de la oferta
    SET @cargo_oferta = (SELECT cargo FROM oferta WHERE id = NEW.id_oferta);

    -- Insertar la notificación para el egresado
    INSERT INTO notificacion (id_usuario, titulo, mensaje, fecha, estado)
    VALUES (
      @egresado_usuario_id,
      'Postulación Avanzada',
      CONCAT('Felicidades, has avanzado a la segunda fase de la oferta para el cargo ', @cargo_oferta, '.'),
      NOW(),
      'No Leída'
    );
  END IF;
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `tr_rechazar_postulacion_notificacion` AFTER UPDATE ON `inscripcion` FOR EACH ROW BEGIN
  ...
  IF NEW.estado = 'Rechazado' AND OLD.estado <> 'Rechazado' THEN
    ...
    VALUES (
      ...
      CONCAT('Tu postulación a la oferta para el cargo ', @cargo_oferta, ' ha sido rechazada.'),
      NOW(),
      'No Leída'
    );
  END IF;
...
INSERT INTO `ubicacion` (`id`, `ciudad`, `departamento`, `direccion`) VALUES
(1, 'Bogotá', 'Cundinamarca', 'Cra 19 No 9-27'),
(2, 'Bogotá', 'Cundinamarca', 'Calle 123 #45-67'),
(3, 'Bogotá', 'Cundinamarca', 'Cra 45 # 15-24'),
(4, 'Bogotá', 'Cundinamarca', 'Calle 123 #45-67'),
(5, 'Bogotá', 'Cundinamarca', 'Calle 123 #45-67'),
(6, 'Bogotá', 'Cundinamarca', 'Calle 123 #45-67'),
(7, 'Caparrapí', 'Cundinamarca', 'Cra 19 #9-27'),
(8, 'Funza', 'Cundinamarca', 'Cra 19 #9-27'),
(9, 'Funza', 'Cundinamarca', 'Cra 19 #9-27');
